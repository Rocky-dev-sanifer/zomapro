...
class ProductComments extends Module
{
	...
	public function hookDisplayProductListReviews($params)
	{
		...
		if (!$this->isCached('module:productcomments/productcomments_reviews.tpl', $this->getCacheId($id_product)))
		{
			require_once(dirname(__FILE__).'/ProductComment.php');
		...
		return $this->fetch('module:productcomments/productcomments_reviews.tpl', $this->getCacheId($id_product));
	}
	...
	public function hookDisplayReviewsProduct($params)
	{
		...
		$this->context->smarty->assign(array(
			...
			'nbComments_extra' => (int)(ProductComment::getCommentNumber((int)Tools::getValue('id_product')))
	   ));
	...
	{
		$this->context->controller->registerStylesheet('modules-productcomments', 'modules/'.$this->name.'/productcomments.css', ['media' => 'all', 'priority' => 150]);
		   $this->context->controller->addJS($this->_path.'js/jquery.malihu.PageScroll2id.min.js');
		...
		{  
			...
			$this->context->controller->registerJavascript('modules-productcomments', 'modules/'.$this->name.'/js/productcomments.js', ['position' => 'bottom', 'priority' => 150]);
		
			if (in_array($this->page_name, array('productscomparison')))
			{
				$this->context->controller->addjqueryPlugin('cluetip');
				$this->context->controller->registerJavascript('modules-productcomments', 'modules/'.$this->name.'/js/products-comparison.js.js', ['position' => 'bottom', 'priority' => 150]);
