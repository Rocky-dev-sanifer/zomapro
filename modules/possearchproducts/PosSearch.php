...
class PosSearch extends ObjectModel
{
    ...
    {
		$psSearchMaxWordLength = (int) Configuration::get('$psSearchMaxWordLength');
            ...
            foreach ($words as $key => $word)
                if (!empty($word) && strlen($word) >= (int)Configuration::get('PS_SEARCH_MINWORDLEN'))
                {
                    ...
                    WHERE sw.id_lang = '.(int)$id_lang.'
                        ...
                        AND sw.word LIKE
                    '.($word[0] == '-'
                            ? ' \''.pSQL(Tools::substr($word, 1, $psSearchMaxWordLength)).'%\''
                            : '\''.pSQL(Tools::substr($word, 0, $psSearchMaxWordLength)).'%\''
                        );
                    if ($word[0] != '-')
                        $score_array[] = 'sw.word LIKE \''.pSQL(Tools::substr($word, 0, $psSearchMaxWordLength)).'%\'';
                }
                else
                    unset($words[$key]);
            if (!count($words))
