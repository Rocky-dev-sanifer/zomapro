...
<div class="pos-menu-vertical" data-more-less="{$menu_config.posvegamenu_more_less}">
	<h3 class="title_vertical hidden-md-down">{$menu_config.posvegamenu_title} </h3>
	<ul class="menu-content"> 
		{foreach from=$menus item=menu name=menus}	 			
				<li class="menu-item menu-item{$menu.id_posvegamenu_item} {$menu.item_class|escape:'html':'UTF-8'} {if count($menu.sub_menu) > 0}hasChild{/if} {if isset($menu.selected_item) && $menu.selected_item == 1}active{/if}">
					<a  style="{if $menu.type_link==2}cursor: auto;{/if}" href="{if $menu.type_link==0}{$menu.link|escape:'html':'UTF-8'}{elseif $menu.type_link ==1}{$menu.custom_link|escape:'html':'UTF-8'}{else}javascript:void(0){/if}" {if $menu.new_window == 1} target="_blank" {/if}>										
						...
						{if $menu.submenu_type == 0 && count($menu.sub_menu) > 0} <i class="hidden-md-down icon_sub ion-ios-arrow-right"></i>{/if}
						{if $menu.submenu_type == 1 && $menu.sub_menu} <i class="hidden-md-down icon_sub ion-ios-arrow-right"></i>{/if}
															...
															{else if $sub_menu_item.type_link == 4}
																<div class="menu-product js-product-miniature"> 
																	...
																	<div class="product_desc">
																		<h3><a class="product_name menu-product-name" href="{$sub_menu_item.product.url}">{$sub_menu_item.product.name}</a></h3>
