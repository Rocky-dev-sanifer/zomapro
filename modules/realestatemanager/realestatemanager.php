...
{
    const AVAILABLE_HOOKS = ['displayHeader', 'moduleRoutes', 'displayCustomerAccount', 'displayTop', 'displayNav1', 'displayNav2', 'displayHome'];
     ...
     * Reproduit la même UX que le formulaire de la page liste :
     * barre de recherche + bouton Filtres qui déplie le panneau complet.
     */
    public function hookDisplayTop($params)
    {
        ...
        $this->context->smarty->assign([
            ...
            're_types'       => RealEstateProperty::getTypes(),
            're_regions'     => RealEstateProperty::getRegions(),
            're_currency'    => Configuration::get('REALESTATE_CURRENCY', 'Ar'),
            're_filters'     => [
    ...
    /**
     * Affiche en page d'accueil les biens marqués "is_home" depuis le BO.
     */
    public function hookDisplayHome($params)
    {
        require_once _PS_MODULE_DIR_ . 'realestatemanager/classes/RealEstateProperty.php';

        $properties = RealEstateProperty::getHomeProperties(6);
        if (empty($properties)) {
            return '';
        }
        ...
        $view_url   = $this->context->link->getModuleLink('realestatemanager', 'view', ['id' => '']);
        ...
        foreach ($properties as $p) {
            ...
            $items[] = [
                ...
                'region_label' => isset($regions[$p['region']]) ? $regions[$p['region']] : $p['region'],
                'price'        => (float)$p['price'],
                'currency'     => $currency,
                'surface'      => (int)$p['surface'],
                'bedrooms'     => (int)$p['bedrooms'],
                'toilets'      => (int)$p['toilets'],
                'parkings'     => (int)$p['parkings'],
                'furnished'    => (int)$p['furnished'],
                'cover'        => !empty($imgs) ? $upload_url . $imgs[0]['filename'] : '',
                'detail_url'   => $view_url . (strpos($view_url, '?') !== false ? '&' : '?') . 'id_property=' . (int)$p['id_property'],
            ];
        }

        $this->context->smarty->assign([
            're_home_items' => $items,
            're_home_list_url' => $this->context->link->getModuleLink('realestatemanager', 'list'),
        ]);

        return $this->display(__FILE__, 'views/templates/hook/display-home.tpl');
    }
