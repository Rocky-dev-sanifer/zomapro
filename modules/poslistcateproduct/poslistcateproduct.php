...
{
	protected $_html = '';
	protected $posidentifier = false;
	public $className;

	protected $default_row = 2;
	protected $default_items = 4;
	protected $default_per_lg = 3;
	protected $default_per_md = 2;
	...
	public function install()
	{
		...
		foreach ($shops as $shop_id)
		{
			...
			$res &= Configuration::updateValue($this->name.'_per_lg', $this->default_per_lg, false, $shop_group_id, $shop_id);
		...
		$res &= Configuration::updateValue($this->name.'_per_lg', $this->default_per_lg);
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName($this->name.'_per_lg');
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitPosProductCates'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res &= Configuration::updateValue($this->name.'_per_lg', (int)Tools::getValue($this->name.'_per_lg'), false, $shop_group_id, $shop_id);
			...
			switch ($shop_context)
			{
				case Shop::CONTEXT_ALL:
					...
					$res &= Configuration::updateValue($this->name.'_per_lg', (int)Tools::getValue($this->name.'_per_lg'));
					...
					if (count($shop_groups_list))
					{
						foreach ($shop_groups_list as $shop_group_id)
						{
							...
							$res &= Configuration::updateValue($this->name.'_per_lg', (int)Tools::getValue($this->name.'_per_lg'), false, $shop_group_id);
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
	   ...
	   $item['description'] = str_replace('/pos_grano/',__PS_BASE_URI__,$item['description']);
	  }
        if (is_array($items))
			foreach ($items as &$item)
			{	
	...
	{	$id_item = Tools::getValue('id_item');
		...
		$helper->module = $this;
		$helper->posidentifier = $this->posidentifier;
	...
	public function getConfigFieldsValues()
	{
		...
		return array(
			...
			$this->name.'_per_lg' => Tools::getValue($this->name.'_per_lg', Configuration::get($this->name.'_per_lg', null, $id_shop_group, $id_shop)),
	...
	{	
		...
		foreach ($languages as $lang)
		{
			...
			$fields['url'][$lang['id_lang']] = Tools::getValue('url_'.(int)$lang['id_lang'], (isset($slide->url[$lang['id_lang']])) ? $slide->url[$lang['id_lang']] : null);
			$fields['description'][$lang['id_lang']] = html_entity_decode(Tools::getValue('description_'.(int)$lang['id_lang'], (isset($slide->description[$lang['id_lang']])) ? $slide->description[$lang['id_lang']]:null));
			$fields['description'][$lang['id_lang']] = str_replace('/pos_grano/',__PS_BASE_URI__,$fields['description'][$lang['id_lang']]);
		}
    ...
    {	
		$req = 'SELECT hs.`id_category`
				...
				WHERE hs.`id_poslistcateproduct_items` = '.(int)$id_item;
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);
		return  (isset($row['id_category'])) ? (int)$row['id_category'] : 0;
    }
    ...
    {	
		$req = 'SELECT hs.`list_subcategories`
				...
				WHERE hs.`id_poslistcateproduct_items` = '.(int)$id_item;
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);
		return (isset($row['list_subcategories']))  ? $row['list_subcategories'] : null;
    }
