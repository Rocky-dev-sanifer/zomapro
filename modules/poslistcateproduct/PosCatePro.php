...
class PosCatePro extends ObjectModel
{
	public $id_category;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'poslistcateproduct_items',
		'primary' => 'id_poslistcateproduct_items',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'poslistcateproduct` (`id_shop`, `id_poslistcateproduct_items`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'poslistcateproduct`
			WHERE `id_poslistcateproduct_items` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE pci.`id_poslistcateproduct_items` = pc.`id_poslistcateproduct_items` AND pc.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT pci.`position` as position, pci.`id_poslistcateproduct_items` as id_item
			FROM `'._DB_PREFIX_.'poslistcateproduct_items` pci
			LEFT JOIN `'._DB_PREFIX_.'poslistcateproduct` pc ON (pci.`id_poslistcateproduct_items` = pc.`id_poslistcateproduct_items`)
			WHERE pc.`id_shop` = '.(int)$id_shop.' AND pci.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new PosCatePro($row['id_item']);
	...
	public static function getAssociatedIdsShop($id_item)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE pc.`id_poslistcateproduct_items` = '.(int)$id_item
		);
